...
class AjaxLoader {
	...
	public function mf_autocomplete_taxonomy_callback( $taxonomy = 'category' ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			...
			$result[] = array(
				'value' => 'nothing-found',
				'label' => esc_html__( 'Nothing Found', 'martfury' )
			);
		}
	...
	public function mf_autocomplete_post_type_callback( $post_type = 'product' ) {
		...
		if ( is_array( $posts ) && ! empty( $posts ) ) {
			foreach ( $posts as $post ) {
				$data          = array();
				$data['value'] = $post->ID;
				$data['label'] = esc_html__( 'Id', 'martfury' ) . ': ' . $post->ID . ' - ' . esc_html__( 'Title', 'martfury' ) . ': ' . $post->post_title;
				$result[]      = $data;
			}
		} else {
			$result[] = array(
	...
	public function mf_autocomplete_taxonomy_render( $taxonomy = 'category' ) {
		...
		foreach ( $terms as $term ) {
			...
			$data[] = sprintf(
				...
				esc_attr( $term->slug ),
				esc_html__( 'Id: ', 'martfury' ),
				esc_html( $term->term_id ),
				esc_html__( 'Name: ', 'martfury' ),
				esc_html( $term->name )
			);
		}
